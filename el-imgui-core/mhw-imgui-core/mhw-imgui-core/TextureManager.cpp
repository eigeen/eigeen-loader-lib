...
TextureHandle TextureManager::register_texture(void* texture) {
    ...
    Logger::debug("Registered texture: handle {}", handle);
...
TextureHandle TextureManager::load_texture(std::string_view path, u32* out_width, u32* out_height) {
    ...
    Logger::debug("Loaded texture: handle {}, path {}", handle, path);
...
void TextureManager::unload_texture(TextureHandle handle) {
    ...
    if (it == m_textures.end()) {
        Logger::error("Failed to unload texture: handle {} does not exist", handle);
        return;
    }
    ...
    Logger::debug("Unloaded texture: handle {}", handle);
}
...
D3D12_GPU_DESCRIPTOR_HANDLE TextureManager::get_gpu_descriptor_handle(TextureEntry& entry) {
    if (m_next_descriptor_index >= DESCRIPTOR_HEAP_SIZE && m_free_descriptor_indices.empty()) {
        Logger::error("Failed to get GPU descriptor handle: descriptor heap is full");
        return { 0 };
    }
