...
TextureManager::ComPtr<ID3D11ShaderResourceView> TextureManager::load_texture11(std::string_view path, u32* width, u32* height) const {
    ...
    if (!fs::exists(file)) {
        Logger::error("Failed to load texture: {} does not exist", path);
        return nullptr;
    }
    ...
    Logger::error("Failed to load texture: unsupported format {}", ext);
    return nullptr;
}
...
void TextureManager::get_texture_dimensions(const ComPtr<ID3D11Resource>& texture, u32* width, u32* height) {
    ...
    if (dim == D3D11_RESOURCE_DIMENSION_TEXTURE2D) {
        ...
        *height = desc.Height;
        ...
        Logger::error("Failed to get texture dimensions: unsupported resource type");
    }
}
...
DXGI_FORMAT TextureManager::get_texture_format(const ComPtr<ID3D11Resource>& texture) {
    ...
    Logger::error("Failed to get texture format: unsupported resource type");
