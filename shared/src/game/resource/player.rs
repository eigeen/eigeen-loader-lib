...
use crate::{
    game::mt_type::{Entity, GameObject, GameObjectExt, Model},
    utility::memory,
};
...
crate::derive_game_object!(Player);
...
impl Player {
    ...
    pub fn info(&self) -> Option<PlayerInfo> {
        let info_addr = memory::get_value_with_offset(
            (self.as_address() + 0xC0) as *const usize,
            &[0x8, 0x78],
        )?;

        Some(PlayerInfo::from_address(info_addr))
    }
...
#[repr(transparent)]
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct PlayerInfo(*mut c_void);

unsafe impl Send for PlayerInfo {}

crate::derive_game_object!(PlayerInfo);
...
impl PlayerInfo {
    ...
    //     let offset = 0x58 + 0x740 * index;
    //     let info_ptr = memory::get_ptr_with_offset(game_export::PLAYER_BASE, &[offset])?;
    ...
    pub fn name(&self) -> &'static str {
        unsafe {
            let name_ptr = (self.as_ptr() as *const i8).byte_add(0x78);

            CStr::from_ptr(name_ptr).to_str().unwrap_or_default()
        }
    }
...
#[repr(transparent)]
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct PlayerShortInfo(*mut c_void);

unsafe impl Send for PlayerShortInfo {}

crate::derive_game_object!(PlayerShortInfo);
...
impl PlayerShortInfo {
    ...
    pub fn name(&self) -> &'static str {
        unsafe {
            let name_ptr = (self.as_ptr() as *const i8).byte_add(0x49);
...
#[repr(transparent)]
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct PlayerWeaponInfo(*mut c_void);

unsafe impl Send for PlayerWeaponInfo {}

crate::derive_game_object!(PlayerWeaponInfo);
