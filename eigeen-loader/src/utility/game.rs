...
pub fn show_system_message(message: &str, flag: i8) {
    // 为了防止panic或消息丢弃，通过检查玩家基址是否为空判断是否进入游戏场景
    // 可能存在不稳定性，请勿依赖该保险措施
    let Some(singleton) =
        SingletonManager::get_ptr_by_name::<c_void>(&SingletonName::WWISE_BGM_MANAGER)
    else {
        return;
    };
    unsafe {
        if !shared::game::chat::is_player_in_scene(singleton) {
            return;
        }
    }
